		...
		this.each(function() {
			...
			$(this).click(function(){
				...
				if ( $(clone).attr('id') ){
					...
					var str = $(clone).attr('id');
					var arreglo = str.split("_");
					var longitud = arreglo.length;
					var newid ="";
					for(i=0;i<longitud;i++){
						if(i==0){
							newid=arreglo[0]+"_"+counter;
						}
						else{
							if(i>1)
								newid=newid+"_"+arreglo[i];
						}
					}					
				...
				$(clone).find('[id]').each(function(){
					...
					for(i=0;i<longitud;i++){
						if(i==0){
							newid=arreglo[0]+"_"+counter;
							newnm=arreglo[0]+"["+counter+"][";
						}
						else{
							if((i>1)&&(i<(longitud-1))){
								newid=newid+"_"+arreglo[i];
								newnm=newnm+arreglo[i]+"_";
							}
							else {
								if(i==(longitud-1))
								{
									newid=newid+"_"+arreglo[i];
									newnm=newnm+arreglo[i]+"]";
								}
							}
						}
					}						
